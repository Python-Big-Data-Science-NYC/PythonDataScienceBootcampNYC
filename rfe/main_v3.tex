...
\begin{document}
	...
	% ============================================================
	% NEW SLIDE: STRATEGIC DECISION - RESPOND TO RFE vs. WITHDRAW & REFILE
	% ============================================================
	
	\begin{frame}{Strategic Decision: Respond to RFE vs. Withdraw \& Refile}
		\scriptsize
		\begin{block}{The Attorney's Advice \& Why It Matters}
			\textbf{Scenario:} USCIS issues an RFE or denial with harsh language (e.g., ``paid profile-building service,'' ``AI-written papers,'' ``fraudulent citations'').
			\vspace{0.2cm}
			\textcolor{red}{\textbf{Why some attorneys recommend WITHDRAW \& REFILE:}}
			\begin{itemize}
				\item The RFE/denial contains \textbf{derogatory comments} that create a negative record.
				\item USCIS officers keep internal track records; the same officer may reject the response.
				\item Allegations of AI-written content or paid authorship are \textbf{extremely hard to rebut} (forensic analysis may be required).
				\item A withdrawal ``clears the record'' - you refile with a clean slate and a different officer.
				\item Success stories exist: many petitioners have withdrawn after harsh RFEs and been approved on refiling.
			\end{itemize}
		\end{block}
		
		\begin{block}{When to Respond to RFE vs. When to Withdraw}
			\centering
			\begin{tabular}{|p{4.5cm}|p{4.5cm}|}
				\hline
				\textcolor{green}{\textbf{RESPOND TO RFE IF:}} & \textcolor{red}{\textbf{WITHDRAW \& REFILE IF:}} \\
				\hline
				Deficiencies are factual (missing forms, vague endeavor) & Allegations of fraud, AI-writing, or paid authorship \\
				\hline
				RFE has neutral/standard language & RFE contains \textbf{derogatory, harsh, or accusatory} language \\
				\hline
				You have strong pre-filing evidence with timestamps & Officer has a known pattern of rejecting similar cases \\
				\hline
				No allegations of bad faith & Attorney tracks that this specific officer denies most responses \\
				\hline
				You have $8k-$15k for attorney response & You have $12k-$18k already paid - attorney advises refile is cheaper/faster \\
				\hline
			\end{tabular}
		\end{block}
	\end{frame}
	
	\begin{frame}{The "Poisoned Record" Problem - Why Attorneys Say Refile}
		\scriptsize
		\begin{block}{What Makes a Record ``Poisoned''?}
			\textbf{Examples of derogatory language that trigger withdraw/refile advice:}
			\begin{itemize}
				\item ``Petitioner appears to have used a \textbf{paid profile-building service} to inflate citations.''
				\item ``The submitted papers bear indicia of \textbf{AI-generated content} without meaningful human authorship.''
				\item ``The expert letters are \textbf{template-based} and lack specific knowledge of petitioner's work.''
				\item ``The citation pattern suggests \textbf{reciprocal citation rings} or manufactured metrics.''
				\item ``Petitioner's claimed research has \textbf{no verifiable impact} on the field.''
			\end{itemize}
			\textcolor{red}{\textbf{Why this is dangerous:}}
			\begin{itemize}
				\item These allegations go to \textbf{credibility}, not just missing evidence.
				\item Responding requires \textbf{forensic evidence} (version history, AI detection reports, raw data).
				\item Even if you win, the negative record stays in USCIS systems.
				\item The \textbf{same officer} will adjudicate your RFE response (no change of reviewer).
			\end{itemize}
		\end{block}
		
		\begin{block}{What Attorneys Track - Officer Behavior}
			\begin{quote}
				``Experienced attorneys keep internal records on USCIS officers. If Officer X has denied 9 out of 10 RFE responses in the last year, withdrawing and refiling to get a different officer is a sound strategy.''
			\end{quote}
			\textcolor{blue}{This is not forum-shopping - it's strategic case management based on empirical data.}
		\end{block}
	\end{frame}
	
	\begin{frame}{Success Stories: Withdraw \& Refile After Harsh RFE}
		\scriptsize
		\begin{block}{Real-World Outcomes (Attorney-Reported)}
			\centering
			\begin{tabular}{|p{3cm}|p{4.5cm}|p{3cm}|}
				\hline
				\textbf{Case Type} & \textbf{RFE/Allegation} & \textbf{Outcome on Refile} \\
				\hline
				EB2-NIW & ``AI-written papers,'' ``paid citations'' & Approved within 4 months (new officer) \\
				\hline
				EB1A & ``Profile-building service,'' ``no original contribution'' & Approved after refile with raw data \\
				\hline
				EB2-NIW & ``Derogatory comments about endeavor'' & Approved - different service center \\
				\hline
				EB1B & ``Fabricated peer review records'' & Withdrew, refiled as EB2-NIW, approved \\
				\hline
			\end{tabular}
			\vspace{0.2cm}
			\textbf{Key insight from attorneys:}
			\begin{quote}
				``If you paid $12k-$18k to an attorney, trust their advice. They know which officers reject cases. Sometimes responding to an RFE is throwing good money after bad. Withdrawing and refiling resets the clock - and the officer.''
			\end{quote}
		\end{block}
	\end{frame}
	
	\begin{frame}{Decision Matrix: RFE Response vs. Withdraw \& Refile}
		\scriptsize
		\begin{block}{Ask These Questions Before Deciding}
			\begin{enumerate}
				\item \textbf{Does the RFE contain accusatory/derogatory language?} 
				\begin{itemize}
					\item YES $\rightarrow$ Strongly consider withdraw/refile
					\item NO (neutral, factual deficiencies) $\rightarrow$ Respond to RFE
				\end{itemize}
				\item \textbf{Does your attorney track this officer's approval rate?}
				\begin{itemize}
					\item Low approval rate on responses $\rightarrow$ Withdraw/refile
					\item Unknown or average $\rightarrow$ Respond to RFE
				\end{itemize}
				\item \textbf{Can you prove the work was done BEFORE filing (timestamps, version history)?}
				\begin{itemize}
					\item YES - strong evidence $\rightarrow$ Respond to RFE
					\item NO or weak evidence $\rightarrow$ Withdraw/refile (build better record)
				\end{itemize}
				\item \textbf{What is the cost difference?}
				\begin{itemize}
					\item Respond to RFE: \$5k-\$15k (attorney + evidence)
					\item Withdraw \& Refile: \$8k-\$12k (new filing + attorney)
					\item If similar cost, refile may be safer
				\end{itemize}
			\end{enumerate}
		\end{block}
		
		\begin{block}{Practical Bottom Line}
			\textcolor{blue}{\textbf{When in doubt, listen to your attorney.}} They have experience with specific officers and service centers that you don't. If they recommend withdraw \& refile after seeing the RFE language, there is usually a good reason - often a ``poisoned record'' that cannot be easily cleaned.
			\vspace{0.2cm}
			\textcolor{red}{Do NOT assume that responding to an RFE is always better. Sometimes the best strategic move is to withdraw, fix the record, and refile fresh.}
		\end{block}
	\end{frame}
